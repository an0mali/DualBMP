...
const PROGMEM char initMes[]  = "Initializing DualBMPs...";
...
float DualBMP::pressure(int32_t b5, int sensnr)
{
  ...
  x1 = (x1 * 3038) >> 16;
  x2 = (-7357 * p) >> 16;
  return (p + ((x1 + x2 + 3791) >> 4))/ 100.0f;//disable /100 to increase accuracy since exact measurments dont matter?
                                               // ^ this increases sensor volatility / 100.0f; // Return pressure in mbar
}
